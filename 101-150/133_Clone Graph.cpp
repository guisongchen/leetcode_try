...
public:
    Node* cloneGraph(Node* node) {
        ...
        // key the node of first layer, then do deep explore neighbors
        queue<Node*> q({node});
        Node* nodeCopy = new Node(node->val);
        hash[node] = nodeCopy;
        ...
        while (!q.empty()) {
            ...
            for (Node* item : root->neighbors) {
                
                // if not copy, do it now
                if (!hash.count(item)) {
                ...
                // now hash[item] exist, add copyed node to neighbors of copyed root
                hash[root]->neighbors.push_back(hash[item]);
            }
        }
